(function(){
  ...
  var Ship = Asteroids.Ship = function (options) {
    // this.laserSound = new Audio("lib/sound/Laser_Shoot74.wav")
    this.img = new Image();
  ...
  Ship.prototype.power = function (impulse) {
    if(impulse === "thrust"){
      if(Asteroids.Util.norm(this.vel) < 12){
        var acclerateSound = document.getElementById("accelerate");
        acclerateSound.play();
  ...
  Ship.prototype.fireBullet = function () {
    ...
    var laserSound = new Audio("lib/sound/Laser_Shoot74.wav")
  ...
  Ship.prototype.collideWith = function (otherObject) {
    ...
    (JSON.stringify(this.pos) != JSON.stringify(this.game.centerPosition()))) {
      var hurtSound = document.getElementById("ship-hit");
      hurtSound.play();
      this.game.lives -= 1;
    }
  };
